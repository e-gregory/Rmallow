...
#' @param use_cpp Logical; if TRUE (default), use the fast C++ implementations.
...
Mallows <- function(datas, G, iter = 100L, hyp = NULL, plot_like = FALSE,
                    verbose = TRUE, tol = 1e-6, use_cpp = TRUE) {
  # Input validation
  ...
  dists_table <- DistanceDistribution(n_items, use_cpp = use_cpp)
  ...
  infos <- KendallInfo(datas, use_cpp = use_cpp)
  ...
  for (i in seq_len(iter)) {
    ...
    z <- EStep(R, datas, p, lambda, G, N, C_lam, all_dists_data, use_cpp = use_cpp)

    # M-Step: update parameters
    R <- UpdateR(datas, z, infos, use_cpp = use_cpp)
    p <- UpdateP(z)

    # Compute distances for lambda update and likelihood
    all_dists_data <- AllKendall(datas, do.call(rbind, R), infos, use_cpp = use_cpp)
