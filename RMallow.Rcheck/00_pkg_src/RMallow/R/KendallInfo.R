...
#' KendallInfo(r_ties)
KendallInfo <- function(r, inds = NULL, use_cpp = TRUE) {
  ...
  if (use_cpp) {
    return(kendall_info_cpp(r, inds))
  }

  # Fallback to R implementation
  KendallInfo_R(r, inds)
}

#' Pure R implementation of KendallInfo (for benchmarking)
#' @keywords internal
KendallInfo_R <- function(r, inds = NULL) {
  if (!is.matrix(r)) {
    r <- as.matrix(r)
    attr(r, "dimnames") <- NULL
  }
