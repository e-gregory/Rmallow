...
DistanceDistribution <- function(N = 3L, use_cpp = TRUE) {
  # Input validation
  ...
  if (N <= 4L) {
    out <- table(SeqDistribution(N, use_cpp = use_cpp))
    return(out)
  }

  # For larger N, use recurrence relation
  # Start with N=4 and build up
  out <- table(SeqDistribution(4L, use_cpp = use_cpp))
  out <- as.numeric(out)

  for (i in 4L:(N - 1L)) {
    out <- NextTable(out, i, use_cpp = use_cpp)
  }
