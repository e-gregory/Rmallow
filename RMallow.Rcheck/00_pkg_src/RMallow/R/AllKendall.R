...
AllKendall <- function(r, seqs, data_info = NULL, use_cpp = TRUE) {
  ...
  if (use_cpp) {
    # Generate column pair indices
    inds <- combn_pairs_cpp(ncol(r))

    # Compute Kendall information for data if not provided
    if (is.null(data_info)) {
      data_info <- kendall_info_cpp(r, inds)
    }

    # Compute Kendall information for reference sequences
    seqs_info <- kendall_info_cpp(seqs, inds)

    # Compute distances using C++
    return(all_kendall_cpp(data_info, seqs_info))
  }

  # Fallback to R implementation
  AllKendall_R(r, seqs, data_info)
}

#' Pure R implementation of AllKendall (for benchmarking)
#' @keywords internal
AllKendall_R <- function(r, seqs, data_info = NULL) {
  if (!is.matrix(r)) {
    r <- as.matrix(r)
  }
  if (!is.matrix(seqs)) {
  ...
  # Compute Kendall information for data if not provided
  if (is.null(data_info)) {
    data_info <- KendallInfo_R(r, inds)
  }

  # Compute Kendall information for reference sequences
  seqs_info <- KendallInfo_R(seqs, inds)
